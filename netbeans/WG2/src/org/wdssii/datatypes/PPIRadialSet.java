...
public class PPIRadialSet extends RadialSet implements Table2DView {
    ...
    /**
     * Export to ESRI shapefile...experimental. FIXME: separate writer package
     * probably
     */
    @Override
    ...
    public void exportSHPPointData(URL aURL) {
        ...
        try {
            ...
            final Location radarLoc = getRadarLocation();
            final double sinElevAngle = getFixedAngleSin();
            final double cosElevAngle = getFixedAngleCos();
            final float firstGateKms = getRangeToFirstGateKms();
            final int maxGateCount = getNumGates();
            final int numRadials = getNumRadials();
            ...
            Coordinate c0, c1, c2, c3, c4;
            c2 = c3 = null;

            // --------------------------------------------------------
            // On first radial, create an attenuation cache...
            Radial firstRadial = (numRadials > 0) ? getRadial(0) : null;
            RadialSetRenderer.RadialATHeightGateCache c =
                    new RadialSetRenderer.RadialATHeightGateCache(this, firstRadial, maxGateCount, sinElevAngle, cosElevAngle);
            ...
            for (Radial r : radials) {
                ...
                float gateWidthKms = getGateWidthKms();
