...
public class RadialSet extends DataType implements Table2DView {
	...
	public void exportToURL(URL aURL, GridVisibleArea g) {
		...
		try {
			...
			String rangeRef = getRowHeader(centerRow);
			...
			out.write(String.format("%s %6.2f\n", rangeRef, azimuthRef));
			...
			out.write(String.format("%6.2f %6.2f\n", azmbeg, azmend));
			...
			// Guessing on output order...
			int linebreak = 0;
			CellQuery q = new CellQuery();
			for (int col = g.startCol; col <= g.lastFullColumn; col++) {
				// Output in increasing range....
				for (int row = g.lastFullRow; row >= g.startRow; row--) {
					this.getCellValue(row, col, q);
					float v = q.value;
					out.write(String.format(" %f", v));
					if (++linebreak > 6) {
						out.write("\n");
						linebreak = 0;
					}
