...
public class LLHAreaFeature extends Feature {
	...
	@Override // superclass should handle this... This breaks points completely if removed.
				// Wow.
	...
	public ArrayList<FeatureRenderer> getNewRendererList(String type, String packageName) {
		...
		// Kinda violates the whole create by name thing... Need a generic way of
		// getting
	...
	public boolean handleMouseMoved(int pickID, GLWorld w, FeatureMouseEvent e) {
		 ...
		 * //LOG.error("Got pick "+pickID); //ArrayList<Integer> picks =
		 * myLLHPolygonRenderer.myPicks; int id = myLLHPolygonRenderer.myFirstPick; int
		 * count = myLLHPolygonRenderer.myPickCount; LOG.error("INFO: "+pickID +", "
		 * +id+", " +(id+count)); boolean found = ((pickID >= id) && (pickID <
		 * (id+count))); if (found) { LOG.error("FOUND ID OF "+pickID); int pointNumber
		 * = pickID-id; LOG.error("PICK INDEX OF "+pointNumber); // Auto select. FIXME:
		 * redesign/streamline // From the LLHAreaLayer stuff. Bleh this needs to
		 * integrate... // Wow needs a GLWorld to create new control points each time...
		 * // for moment hack into renderer...clean me clean me clean me.. // (got into
		 * this mess because of worldwind) ArrayList<LLHAreaControlPoint> c =
		 * myLLHPolygonRenderer.myLastControls; LLHAreaControlPoint controlPoint =
		 * c.get(pointNumber);
		 * 
		 * if (myLLHArea instanceof LLHAreaSet) { // BLEH! LLHAreaSet set = (LLHAreaSet)
		 * (myLLHArea); int index = controlPoint.getLocationIndex(); PointSelectCommand
		 * command = new PointSelectCommand(set, index);
		 * CommandManager.getInstance().executeCommand(command, true); return true; } }
		 */
	...
	public boolean handleMouseDragged(int pickID, GLWorld w, FeatureMouseEvent e) {
		V3 aPoint = w.project2DToEarthSurface(e.x, e.y, V3.EARTH_RADIUS_KMS);
		...
		if (myControlPoint != null) {
			...
			// Get current 3d point in lat, lon, height....this seems to be to get the
			// current elevation
			...
			// Current control point forced to earth surface
			V3 newPoint = w.project2DToEarthSurface(e.x, e.y, 0);
			...
			if (newPoint == null) {
				...
				if (myLLHArea instanceof LLHAreaSet) {
					...
					ArrayList<LLD_X> list = ((ArrayList<LLD_X>) m
							.getPropertyValue(LLHAreaSet.LLHAreaSetMemento.POINTS));
					if (list != null) {
	 ...
	 * Point mousePoint) {
	 * 
	 * // Include this test to ensure any derived implementation performs it. if
	 * (this.getAirspace() == null) { return; }
	 * 
	 * // Get current 3d point in lat, lon, height....this seems to be to get the
	 * current elevation V3 controlPointL =
	 * w.projectV3ToLLH(controlPoint.getPoint());
	 * 
	 * // Find new 3D point on earth surface using this elevation... V3 newPoint =
	 * w.project2DToEarthSurface(mousePoint.getX(), mousePoint.getY(),
	 * controlPointL.z); if (newPoint == null) { return; }
	 * 
	 * // And make it lat, lon, height... V3 newPosition =
	 * w.projectV3ToLLH(newPoint);
	 * 
	 * int index = controlPoint.getLocationIndex(); LLHArea area =
	 * this.getAirspace();
	 * 
	 * if (area instanceof LLHAreaSet) { LLHAreaSet set = (LLHAreaSet) (area);
	 * FeatureMemento m = set.getMemento(); // vs getNewMemento as in gui //
	 * control...hummm // currently copying all points into 'points'
	 * 
	 * @SuppressWarnings("unchecked") ArrayList<LLD_X> list = ((ArrayList<LLD_X>)
	 * m.getPropertyValue(LLHAreaSet.LLHAreaSetMemento.POINTS)); if (list != null) {
	 * 
	 * // Copy the location info at least... LLD_X oldOne = list.get(index); LLD_X
	 * newOne = new LLD_X(newPosition.x, newPosition.y, oldOne);
	 * 
	 * list.set(index, newOne);
	 * 
	 * FeatureMemento fm = (FeatureMemento) (m); // Check it FeatureChangeCommand c
	 * = new FeatureChangeCommand(area.getFeature(), fm);
	 * CommandManager.getInstance().executeCommand(c, true); } } }
	 */
