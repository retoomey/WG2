...
public class Product {
	...
	/**
	 * The datatype for this product such as 'Reflectivity' The product does
	 * not have to be loaded for this information
	 */
	...
	/**
	 * Called on the product with a DataRequest which is just a
	 * Future<DataType> without using Future since I couldn't get it to
	 * work.. :)
	 *
	 * @param theRequest the data request that will have our datatype once
	 * loaded
	...
	/**
	 *
	 * @return if this product fills the physical space around radar. Thus,
	 * any other product that alsos fills the space cannot be shown at same
	...
	/**
	 * Thie function requires render order to be changed depending on
	 * selection, the selected object must be drawn after the others..
	 * bleh... FIXME?
	 *
	 * @param f
	 * @return
	 ...
	 * Typically this is the latest(n) time records at our subtype. Count is
	 * the number of records wanted
	 *
	...
	public static class ProductLoopRecord {
		...
		/**
		 * The list of records for looping on this product. Note product
		 * might eventually not use this
		 */
	...
	/**
	 * Get the records for looping upon for this product. We use the
	 * IndexRecord here, but eventually we could remove this dependency if
	 * needed.
	 *
...
// Stock helper objects -------------------------------------------------------
	...
	/**
	 * Create a navigator for this product. ProductFeature uses this to get
	 * our navigator. Note it will return NULL until the product DataType is
	 * loaded.
	 */
	 ...
	 * loaded into memory, typically it starts as Dimensionless
	 *
	 * @return
	 */
	...
	/**
	 * Generate a color map based on data values. This is currently used
	 * when colormap is missing. Subclasses can override to make a map
	 * better based on class (RadialSet, etc.)
	 *
	 ...
	 *
	 * @param classSuffix
	 * @param useBaseClass Fall back to 'base' class if special class
	 * missing. For example, no "RadialSetNavigator" will use
	 * "ProductNavigator"
	 * @param cachePerProduct Store an instance per product object, for
	 * example RadialSetRenderer is cached per product
	 * @param root
	 * @param extrainfo
	 * @return
	 */
	...
	/**
	 * Get the info string. This is displayed by the navigation view when
	...
	/**
	 * Sort the given product list into volume order. For RadialSets, this
	 * would be the elevation order.
	 *
	 * Note: This function assumes every product in the list is the same
	 * datatype This should always be the case, mixing different products in
	 * a volume currently would be strange.
	 *
	...
	/**
	 * Create the default cache key. The record selector uses this to get
	 * the default key for a regular product. The default product internal
	 * creation uses this key as well. Regular products at least have one
	 * index and record per product
	 *
	...
	/**
	 * Get the product in the given navigation direction, if any. FIXME:
	...
	/**
	 * Peek in a navigation direction and get the record for it. Note since
	 * a product could exist without an Index or IndexRecord this may not be
	 * valid for all types of products.
	 *
