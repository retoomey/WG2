...
/**
 *  A MouseAdapter for a table using a ListSelectionModel.  Handles
 * standard stuff like a popup menu and figuring out model object per
 * table line.
 * This class will replace RowEntryTableMouseAdapter...
 * 
 ...
 */
public class RowMouseAdapter extends MouseAdapter {
    ...
    public RowMouseAdapter(JTable table, TableModel model) {
        myTable = table;
        ...
        if (e.getComponent().isEnabled()
            ...
            if ((row > -1) && (column > -1)) {
                ...
                if (e.getClickCount() == 1){
                     handleClick(orgRow, orgColumn);
                }else{
                    handleDoubleClick(orgRow, orgColumn);
                }
            }
        }
    }
    ...
    private void showPopup(MouseEvent e) {
        if (e.isPopupTrigger()) {
            ...
            if ((row > -1) && (column > -1)) {
                int orgColumn = myTable.convertColumnIndexToModel(column);
                int orgRow = myTable.convertRowIndexToModel(row);
                JPopupMenu popupmenu = getDynamicPopupMenu(orgRow, orgColumn);
                if (popupmenu != null){
