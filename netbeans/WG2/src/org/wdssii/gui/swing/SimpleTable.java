...
public class SimpleTable extends JLabel
	...
	public static class SimpleTableModel {
		...
		public void hitTest(int x, int y, Point out) {
			final int w = getCellWidth();
			final int h = getCellHeight();
			int col = x / w;
			int row = y / h;

			// Don't pin, selection needs maximum
			// Selection 'hit'
			// if (col > getNumCols(){
			// col = getNumCols();
			//}
	...
	private Point startCell = new Point();
	...
	public void mousePressed(MouseEvent e) {
		...
		if (myMode == 1) {
			if (myModel != null) {
				myModel.hitTest(e.getX(), e.getY(), startCell);
				if (startCell.x > -1) {
					if (startCell.y > -1) {
						myModel.selectRange(startCell.x, startCell.y, startCell.x, startCell.y);
						repaint();
					}
				}
			}
	...
	public void mouseDragged(MouseEvent e) {
		...
		if (myMode == 0) { // move mode
			...
			if (myModel != null) {
				...
				myModel.hitTest(e.getX(), e.getY(), endCell);
				if (startCell.x > -1) {
					if (startCell.y > -1) {
