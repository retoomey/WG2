...
import org.wdssii.gui.volumes.LLHAreaLayerController.MouseMode;
...
public class LLHAreaSetGUI extends FeatureGUI {
	...
	private void setupComponents() {
		...
		aButton.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				LLHAreaLayerController l = CommandManager.getInstance().getLLHController();
				if (l != null) {
					l.setMouseMode(MouseMode.MOUSE_ADD_REMOVE);
					updateToolbar();
				}
			}
		});
		...
		Icon i2 = SwingIconFactory.getIconByName("cart_add.png");
		...
		bButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				LLHAreaLayerController l = CommandManager.getInstance().getLLHController();
				if (l != null) {
					l.setMouseMode(MouseMode.MOUSE_NORMAL);
					updateToolbar();
				}
			}
		});
	...
	public void updateToolbar() {
		LLHAreaLayerController l = CommandManager.getInstance().getLLHController();
		MouseMode m = MouseMode.MOUSE_INACTIVE;
		if (l != null) {
			m = l.getMouseMode();
		}
		if (m == MouseMode.MOUSE_ADD_REMOVE) {
			myAddRemoveButton.setSelected(true);
		} else {
			myAddRemoveButton.setSelected(false);
		}
		if (m == MouseMode.MOUSE_NORMAL) {
	...
	/**
	 * Here is how you can use a SimpleFeatureType builder to create the
	 * schema for your shapefile dynamically. <p> This method is an
	 * improvement on the code used in the main method above (where we used
	 * DataUtilities.createFeatureType) because we can set a Coordinate
	 * Reference System for the FeatureType and a a maximum field length for
	 * the 'name' field dddd
	 */
