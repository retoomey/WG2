...
public class BinarySearchTree< Key, Value> implements Iterable< BinarySearchTree.Node< Key, Value>> {
    ...
    public static class Node< Key, Value> {
        ...
        public void updateDataFromChildren() {
            subtreeSize = 1;
            if (left != null) {
                subtreeSize += left.getSubtreeSize();
            }
            if (right != null) {
                subtreeSize += right.getSubtreeSize();
            }
        }
    ...
    /**
     * Will throw {@link ClassCastException} if the keys are not
     * {@link Comparable}
     */
     ...
     *
     * If the new node overwrites and old node, that old node's value is
     * returned.
     */
    ...
    /**
     * Attempts to remove the Node from the tree. Returns the previous value of
    ...
    protected Value insertBelowNode(Node< Key, Value> node, Node< Key, Value> parent) {
        while (true) {
            ...
            if (cmp == 0) {
                return parent.setValue(node.value);
    ...
    protected void updateAfterRotate(Node< Key, Value> node0, Node< Key, Value> node1) {
        if (node0 != null) {
            node0.updateDataFromChildren();
        }
        if (node1 != null) {
            node1.updateDataFromChildren();
        }
    }
    ...
    protected class NodeIterator implements Iterator< Node< Key, Value>> {
        ...
        public void remove() {
            ...
            if (lastReturned.hasBothChildren()) {
                next = lastReturned;
            }
